...
class User < ActiveRecord::Base
  ...
  def crawl_friend_checkins
    ...
    query = '{"checkin": "select checkin_id, message, author_uid, target_type, target_id, page_id, timestamp from checkin where author_uid IN (SELECT uid2 FROM friend WHERE uid1 = ' + self.user_hash["id"] + ') and message != ' + "''" + ' order by timestamp desc limit 50", "user": "select username, name, id, pic from profile where id in (select author_uid from #checkin)", "page": "select name, page_id, location from page where page_id in (select page_id from #checkin)"}'
